...
export default function MasterBUPage() {
  ...
  const onSubmit = async () => {
    if (!name.trim()) {
      showFeedback("BU Name wajib diisi.", "Validasi");
      return;
    }

    setSubmitting(true);
    if (!editing) {
      const result = await createBusinessUnitMaster({ name: name.trim() });
      setSubmitting(false);
      if (!result.success) {
    ...
    <>
      ...
      <section className={styles.panel}>
        ...
        <div className={styles.uploadNote}>
          Format file: Excel (.xlsx/.xls). Kolom minimal: BU Name, Status. BU Code di-generate otomatis.
        </div>
      </section>
